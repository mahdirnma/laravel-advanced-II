                ...
                <table class="w-full min-h-full border border-gray-400">
                    ...
                    @foreach($products as $product)
                        <tr>
                            ...
                            <td class="text-center">
                                <form action="{{route('products.show',compact('product'))}}" method="get">
                                    @csrf
                                    <button type="submit" class="text-fuchsia-600">sales</button>
                                </form>
                            </td>
                            <td class="text-center">{{number_format($product->profit)}}</td>
                            <td class="text-center">{{number_format($product->price)}}</td>
                            <td class="text-center">{{$product->ingredients->description}}</td>
